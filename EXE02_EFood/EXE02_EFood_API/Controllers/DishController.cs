...
namespace EXE02_EFood_API.Controllers
{
    ...
    public class DishController : ControllerBase
    {
        ...
        [HttpGet("/api/restaurant/{resId}/dish")]
        ...
        public IActionResult ReviewOfDish()
        {
            ...
            {
                result.Add(new ReviewOfDishApiModel { ReviewId = item.ReviewId, DishName = item.Dish.Name,
                    Comment = item.Comment, Time = item.Time.Value.ToString(), UserFullName = item.User.Name,
                    Voting = item.Voting });
            }
            return Ok(result);
        }
        ...
        public IActionResult GetReviewDishById(int? dishId)
        {
            ...
            {
                results.Add(new ReviewOfDishApiModel { ReviewId = item.ReviewId, DishName = item.Dish.Name, 
                    Comment = item.Comment, Time = item.Time.Value.ToString(), UserFullName = item.User.Name, 
                    Voting = item.Voting });
            }
